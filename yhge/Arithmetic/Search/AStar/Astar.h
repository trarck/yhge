...
public:
	...
	typedef Map<int, AstarNode*> OneMap;
	typedef std::unordered_map<int, OneMap> AstarOpenMap;
	typedef Vector<AstarNode*> AstarOpenSeq;
	typedef std::unordered_map<int, std::unordered_map<int,int> > AstarCloseMap;

	typedef std::vector<Point> AstarPointVector;
     ...
     * 取得搜索结果
     */
	AstarPointVector getPath();
	AstarPointVector getPathWithStart();
	AstarPointVector getPathWithEnd();
	AstarPointVector getPathWithStartEnd();
...
protected:
    ...
    //开启队列
	AstarOpenSeq _openSeq;//开启队列，存放每个结点。结点属性,parent 父结点，x,y地图坐,f,g,h路径评分
	...
	AstarOpenMap _opens;//开起列表,设置地图上的坐标，表示开启状态
	AstarCloseMap _closes;//关闭列表,存放已经到达过的点
};
