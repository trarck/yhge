...
bool SceenCoordMoveComponent::registerMessages()
{
    if(Component::registerMessages()){
        ...
        messageManager->registerReceiver(m_owner, MSG_MOVE_DIRECTION, NULL, message_selector(SceenCoordMoveComponent::onMoveDirection),this);
        messageManager->registerReceiver(m_owner, MSG_MOVE_STOP, NULL, message_selector(SceenCoordMoveComponent::onMoveStop),this);
        messageManager->registerReceiver(m_owner, MSG_MOVE_TO, NULL, message_selector(SceenCoordMoveComponent::onMoveTo),this);
...
{
    CCAssert(m_moveType==kMoveTo, "SceenCoordMoveComponent::continueMoveTo before move type is not the same");
...
{
    CCAssert(m_moveType==kMoveDirection, "SceenCoordMoveComponent::continueMoveWithDirection before move type is not the same");
	prepareDirection(direction);
    if (checkMoveable()) {
        prepareMove();
    }else{
...
{
    CCAssert(m_moveType==kMovePath, "SceenCoordMoveComponent::continueMoveWithPaths before move type is not the same");
...
void SceenCoordMoveComponent::startMoveUpdateSchedule()
{
    ...
    {
        case kMoveDirection:
            m_update=schedule_selector(SceenCoordMoveComponent::updateDirection);
            break;
        case kMovePath:
            m_update=schedule_selector(SceenCoordMoveComponent::updatePath);
            break;
        default:
            break;
    }
