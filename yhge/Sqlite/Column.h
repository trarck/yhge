#ifndef YHGE_SQLITE_COLUMN_H_
#define YHGE_SQLITE_COLUMN_H_
...
public:
     ...
     *
     * @param[in] stmtPtr  Shared pointer to the prepared SQLite Statement Object.
     * @param[in] index    Index of the column in the row of result
     */
    Column(Statement::Ptr& stmtPtr, int index); // nothrow
    /// @brief Simple destructor
    virtual ~Column(void)                        ; // nothrow
     ...
     */
    const char*     getName (void) const ; // nothrow
     ...
     */
    const char*     getText  (void) const ; // nothrow
    /**
     ...
     *          thus you must copy it before using it beyond its scope (to a std::string for instance).
     */
    const void*     getBlob  (void) const ; // nothrow
