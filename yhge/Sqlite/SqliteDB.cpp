...
void SqliteDB::connect(const std::string& dbPath,const int flag)
{
    ...
    if (SQLITE_OK != ret)
    {
        ...
        CCLOGERROR("SqliteDB::open %s err:%s",dbPath.c_str(),strerr.c_str());
...
int SqliteDB::execute(const char* query)
{
...
Column SqliteDB::fetchFirstColumn(const char* sqlStr)
{
...
bool SqliteDB::tableExists(const std::string& tableName)
{
...
void SqliteDB::check(const int ret) const
{
    if (SQLITE_OK != ret)
    {
        CCLOGERROR("SqliteDB::check err:%s",sqlite3_errmsg(m_db));
    }
}

void SqliteDB::createFunction(const char*    funcName,
                           int                  arg,
                           bool                 deterministic,
                           void*                app,
